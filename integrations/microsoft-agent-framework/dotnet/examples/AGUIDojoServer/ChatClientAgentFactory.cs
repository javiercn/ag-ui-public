...
internal sealed class ChatClientAgentFactory
{
    ...
    {
        var draftingTool = AIFunctionFactory.Create(
            ([Description("The document topic")] string topic) => DraftDocument(topic),
            name: "draft_document_outline",
            description: "Produce placeholder sections that can be streamed as predictive updates.",
            serializerOptions: AGUIDojoServerSerializerContext.Default.Options);

        return CreateAgent(
            name: "maf-predictive-state",
            description: "Stream predictive updates while editing a knowledge-base article. Use draft_document_outline to prepare the structure and then refine each section.",
            tools: [draftingTool]);
    }
